...
# ---------------------------
# Define networks
inp_up, out_up = DataConsistentNetwork(Radon, Radon.adjoint).network_residual((n_theta, n_s, 1), steps=3, filters=32,
...
def plot_validation(y_in, y_pred, y_true, epoch=10):
    ...
    for i in range(n_images):
        ...
        fig.savefig('images/UpsampleNoisefreeValidationImage_Epoch' + str(epoch) + '_' + str(i) + '.pdf', format='pdf')
        fig.clf()
        ...
        fig.savefig('images/UpsampleNoisefreeReconImage_Epoch' + str(epoch) + '_' + str(i) + '.pdf', format='pdf')
        fig.clf()
    pass
...
DG = DataGenerator(operator=RadonSparse, operator_up=Radon, sigma=0.0)
...
im = axs[0].imshow(y_t[0, ..., 0], cmap='bone')
axs[0].axis('off')
axs[0].set_title('True')
fig.colorbar(im, ax=axs[0])

im = axs[1].imshow(y_n[0, ..., 0], cmap='bone')
axs[1].set_aspect(n_s/n_theta)
axs[1].axis('off')
axs[1].set_title('Input')
fig.colorbar(im, ax=axs[1])

plt.savefig("images/UpsamplingNoisefreeInputTest.pdf", format='pdf')
plt.clf()

# ---------------------------
saver = tf.train.Saver()
save_path = "models/upsampling_noisefree/upsampling_network"
n_save = 1
n_val = 1
n_plot = 1
